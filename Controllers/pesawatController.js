...
const createBookingFlight = async (req, res) => {
  try {
    ...
    if (!req.body || !req.body.flight_id ) {
    ...
    const existingFlight = await Flight.findByPk(req.body.flight_id);
    if (!existingFlight) {
    ...
    return res.status(201).json({ success: true, message: 'Tiket Pesawat berhasil di booking.', data: newBooking });
  } catch (error) {
